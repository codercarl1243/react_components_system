...
`Stack` already handles the cases where a vertical layout surface is needed. `Inline` handles flow-based horizontal grouping. The horizontal *layout surface* case — toolbars, headers, split layouts — consistently needs enough context-specific behaviour that a generic primitive would either be too rigid or too leaky to be useful.
